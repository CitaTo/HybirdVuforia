...
public class HybridPlaneManager : MonoBehaviour, ITrackableEventHandler
{
    ...
    void LateUpdate()
    {
        ...
        Debug.Log("LateUpdate " + imageTrackable.TrackableName + " found at:" + imageTrackable.transform.position.ToString());
    }
    ...
    public void HandleAutomaticHitTest(HitTestResult result)
    {
        ...
        switch (planeMode)
        {
            case PlaneMode.GROUND:
                //if (IsTrackingSticker == false)
                    //return;
                Debug.Log(" OnTrackableStateChanged m_PlaneAugmentation.transform.position: " + m_PlaneAugmentation.transform.position);
                float distanceFromCenter = Vector3.Distance(result.Position, imageTrackable.transform.position);
                if (distanceFromCenter > 0.001)
                {
                    Debug.Log(" OnTrackableStateChanged result.Position: " + result.Position + "distanceFromCenter: +" + distanceFromCenter);
                    //return;
                }
                ...
                m_PlaneAugmentation.transform.position = imageTrackable.transform.position;
                m_PlaneAugmentation.transform.rotation = imageTrackable.transform.rotation;
	...
	public void OnTrackableStateChanged(TrackableBehaviour.Status previousStatus, TrackableBehaviour.Status newStatus)
    {
        ...
        {
            Debug.Log("OnTrackableStateChanged " + imageTrackable.TrackableName + " found at:" + imageTrackable.transform.position.ToString());
            ...
            IsTrackingSticker = true;
        }
        else
        {
            IsTrackingSticker = false;
        }
    }
